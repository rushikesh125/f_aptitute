...
export class ConfigHelper extends EventEmitter {
  ...
  private defaultConfig: Config = {
    ...
    extractionModel: "gemini-2.5-flash-lite-preview", // Default to Flash Lite for faster responses
    solutionModel: "gemini-2.5-flash-lite-preview",
    debuggingModel: "gemini-2.5-flash-lite-preview",
    language: "python",
    opacity: 1.0
  };
  ...
  private sanitizeModelSelection(model: string, provider: "openai" | "gemini" | "anthropic"): string {
    if (provider === "openai") {
      ...
      const allowedModels = ['gemini-1.5-pro', 'gemini-2.0-flash', 'gemini-2.5-flash-lite-preview'];
      if (!allowedModels.includes(model)) {
        console.warn(`Invalid Gemini model specified: ${model}. Using default model: gemini-2.5-flash-lite-preview`);
    ...
    try {
      ...
      if (updates.apiProvider && updates.apiProvider !== currentConfig.apiProvider) {
        if (updates.apiProvider === "openai") {
          ...
          updates.extractionModel = "gemini-2.5-flash-lite-preview";
          updates.solutionModel = "gemini-2.5-flash-lite-preview";
          updates.debuggingModel = "gemini-2.5-flash-lite-preview";
        }
      }
